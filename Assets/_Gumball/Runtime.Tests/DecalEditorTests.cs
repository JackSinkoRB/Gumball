...
namespace Gumball.Runtime.Tests
{
    public class DecalEditorTests : IPrebuildSetup, IPostBuildCleanup
    {
        ...
        {
            Debug.Log("[DECAL EDITOR TEST] One time set up");
        ...
        public void SetUp()
        {
            Debug.Log("[DECAL EDITOR TEST] Setup");
        ...
        {
            Debug.Log("[DECAL EDITOR TEST] Unity tear down");
            yield return DecalEditor.Instance.EndSession();
        }
        
        private void OnSceneLoadComplete(AsyncOperation asyncOperation)
        {
            Debug.Log("[DECAL EDITOR TEST] Scene loading completed");
            CoroutineHelper.Instance.StartCoroutine(WarehouseManager.Instance.SpawnCar(0, 0, 
                Vector3.zero, 
                Quaternion.Euler(Vector3.zero), 
                    (car) =>
                    {
                        Debug.Log("[DECAL EDITOR TEST] Initialised started");
                        WarehouseManager.Instance.SetCurrentCar(car);
                        isInitialised = true;
                        Debug.Log("[DECAL EDITOR TEST] Initialised complete");
                    }));
        }
        ...
        {
            Debug.Log("[DECAL EDITOR TEST] DecalEditorIsSetup 1");
            yield return new WaitUntil(() => isInitialised);
            Debug.Log("[DECAL EDITOR TEST] DecalEditorIsSetup 2");
            Assert.IsTrue(DecalEditor.ExistsRuntime);
            Debug.Log("[DECAL EDITOR TEST] DecalEditorIsSetup 3");
        }
        //
        // [UnityTest]
        // [Order(2)]
