...
namespace Gumball
{
    ...
    public class MagneticScroll : MonoBehaviour, IDragHandler, IPointerDownHandler, IPointerUpHandler
    {
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if (logger != null) logger.Log("Dragged " + gameObject.name + ": " + movement);
        }
        ...
        {
            if (logger != null) logger.Log("Attempting to select icon " + icons.IndexOf(iconToSelect));
        ...
        {
            if (logger != null) logger.Log("Snapping " + iconToSnapToMiddle.gameObject.name + " to magnet");
        ...
        public void OnStartScrolling()
        {
            ...
            if (logger != null) logger.Log("Started scrolling " + gameObject.name);
        }
        ...
        public void OnStopScrolling()
        {
            ...
            if (logger != null) logger.Log("Stopped scrolling " + gameObject.name);
        }
        ...
        {
            if (logger != null) logger.Log((enable ? "Enabling" : "Disabling") + " infinite scroll on " + gameObject.name + ".");
        ...
        private void LoadItemsIntoIcons(int itemStartIndex)
        {
            ...
            {
                if (logger != null) logger.Log(gameObject.name + " doesn't have enough items to fill all the icons!");
        ...
        private void ResetIndexPositions(int itemStartIndex = 0)
        {
            ...
            if (logger != null) logger.Log("Reset index positions for " + gameObject.name + " to " + firstIconIndex + "-" + lastIconIndex + " / " + firstItemIndexShowing + "-" + lastItemIndexShowing);
        }
     ...
     */
        private void OnMovement(Vector2 offset)
        {
            ...
            if (logger != null)
                logger.Log("(" + gameObject.name + ") Movement: " + offset + "(total = " + movementOffset + ")");
        ...
        private IEnumerator SnapToItemOffscreen(int itemIndex, SnapToItemDirection directionToMove)
        {
            ...
            for (int count = 0; count <= items.Count - 1; count++) //we want to repeat until true, but to avoid infinite loop just check the max amount (item count)
            {
                ...
                yield return new WaitUntil(() => lastTrackedSelectedIndex != lastSelectedItemIndex);
        ...
        private void OnStopDecelerating()
        {
            ...
            if (useMagnetSnapping || currentElasticityModifier > 0)
            {
        ...
        private void CheckToWrap(ScrollIcon icon)
        {
            ...
            if (IsPastMinPos(existingPos))
            {
                ...
                if (logger != null) logger.Log(icon.gameObject.name + " is wrapping from bottom to top in " + gameObject.name + " (item " + firstItemIndexShowing + " in " + firstIconIndex + ")");
            ...
            else if (IsPastMaxPos(existingPos))
            {
                ...
                if (logger != null) logger.Log(icon.gameObject.name + " is wrapping from top to bottom in " + gameObject.name + " (item " + lastItemIndexShowing + " in " + lastIconIndex + ")");
        ...
        private float GetElasticity(float originalAmount)
        {
            ...
            if (logger != null) logger.Log("Elasticity modifier is " + finalMovement + " (original = " + originalAmount + ")");
