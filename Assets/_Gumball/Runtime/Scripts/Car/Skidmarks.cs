...
// Copyright 2017 Nition, BSD licence (see LICENCE file). http://nition.co
    public class Skidmarks : MonoBehaviour
    {
        ...
        const float MIN_DISTANCE = 0.5f; // Distance between skid texture sections in metres. Bigger = better performance, less smooth
        ...
        public Vector3 GetSkidMark(int ID)
        {
            int IDtoGet = ID % MAX_MARKS;
            if (IDtoGet < skidmarks.Length)
            {
                return skidmarks[IDtoGet].Pos;
            }

            return skidmarks[skidmarks.Length - 1].Pos;
        }
