...
namespace Gumball
{
    ...
    public class Wheel : MonoBehaviour
    {
        ...
        public bool isLeft; //needs to have x inverted
        ...
        public float camber
        {
            get { return transform.localEulerAngles.z; } // get method
            set
            {
                Transform WL = transform;
                if (this.name.IndexOf("FR") > 0 || this.name.IndexOf("RR") > 0)
                {
                    transform.localEulerAngles = new Vector3(0, 0, -1 * value);
                }

                if (this.name.IndexOf("FL") > 0 || this.name.IndexOf("RL") > 0)
                {
        ...
        public float offset;
        public float initialSusOffset;
        public float initialXOffset;

        private void Awake()
        {
            initialSusOffset = transform.localPosition.y;
        ...
        void FixedUpdate()
        {
            ...
            {
                groundName = hit.collider.gameObject.tag; //Opportunity to change this (Physics Materials????)
            ...
            if (skid != null && Mathf.Abs(slipRatio) > 0.2)
            {
                ...
                //Vector3 skidPoint = hit.point + 4 * (skidCaller.GetComponent<Rigidbody>().velocity) * Time.deltaTime;
                Vector3 skidPoint = hit.point;
                ...
                if (lastSkid != -1)
                {
                    if (Vector3.Distance(skid.GetSkidMark(lastSkid), skidPoint) > 1)
                    {
                        lastSkid = skid.AddSkidMark(skidPoint, hit.normal, Mathf.Abs(slipRatio) - 0.2f, lastSkid);
                    }
                }
                else
                {
                ...
                if (Mathf.Abs(slipRatio) > 0.5f)
                {
                    if (manager)
                    {
                        ...
                        //controller.status.sfxController.SetSkid(true, Mathf.Abs(slipRatio) * 0.9f);
                    }
                }
                else
                {
                ...
                //Look at varying this based on sliding vs straight and some lifetime adjustments.
            }
            else
            {
                lastSkid = -1;
            ...
            if (compression >= 0.9f)
            {
                //controller?.status.sfxController.PlayBottomOut(transform.position);
            }
            ...
            if (localVelo.magnitude > 0.05f)
            {
                rotation += angularVelocity / 2 * Time.deltaTime;
            }
            ...
            if (model != null)
            {
                ...
                model.transform.localRotation =
                    Quaternion.Euler(Mathf.Rad2Deg * rotation, maxSteeringAngle * steering, 0);
                //if (thisname.IndexOf("FR") > 0 || thisname.IndexOf("RR") > 0) model.transform.localPosition = new Vector3(model.transform.localPosition.x + offset, model.transform.localPosition.y, model.transform.localPosition.z);
                //if (thisname.IndexOf("FL") > 0 || thisname.IndexOf("RL") > 0) model.transform.localPosition = new Vector3(model.transform.localPosition.x - offset, model.transform.localPosition.y, model.transform.localPosition.z);
            }

            if (brakes != null)
            {
                if (thisname.IndexOf("FR") > 0)
                    brakes.transform.localPosition = Vector3.up * (compression - 1.0f) * suspensionTravel +
                                                     new Vector3(-0.05f, 0f, 0f);
                else if (thisname.IndexOf("FL") > 0)
                    brakes.transform.localPosition = Vector3.up * (compression - 1.0f) * suspensionTravel +
                                                     new Vector3(0.05f, 0f, 0f);
                else if (thisname.IndexOf("RR") > 0)
                    brakes.transform.localPosition = Vector3.up * (compression - 1.0f) * suspensionTravel +
                                                     new Vector3(-.0f, 0f, 0f);
                else if (thisname.IndexOf("RL") > 0)
                    brakes.transform.localPosition = Vector3.up * (compression - 1.0f) * suspensionTravel +
                                                     new Vector3(.0f, 0f, 0f);
                brakes.transform.localRotation = Quaternion.Euler(0, maxSteeringAngle * steering, 0);
            }
        }
